...
def print_datetime(label="Current Date and Time"):
    """Imprime a data e hora atual com um rótulo opcional."""
    data_hora_atual = datetime.datetime.now()
    data_hora_formatada = data_hora_atual.strftime("%d/%m/%Y %H:%M:%S")
    print(label + ":", data_hora_formatada)

def check_if_gpu_available():
    """Verifica e imprime informações sobre GPUs disponíveis."""
...
def check_if_set_seed(seed=None):
    """Define a semente para geração de números aleatórios, se fornecida."""
    if seed:
...
def create_dirs(directories):
    """Cria diretórios da lista fornecida, se ainda não existirem."""
    for directory in directories:
        os.makedirs(directory, exist_ok=True)

def dataloader(directory, image_size, batch_size):
    """Carrega e transforma o conjunto de dados a partir de um diretório especificado."""
...
def weights_init(m):
    """Inicializa os pesos para camadas Conv e BatchNorm de uma rede neural."""
...
def load_checkpoint(path, generator, discriminator, optim_g, optim_d):
    """Carrega um ponto de controle (checkpoint) do modelo a partir de um caminho especificado."""
...
def plot_losses(losses_g, losses_d, save_plot_image):
    """Plota as perdas do gerador e discriminador e salva a imagem do gráfico."""
